  ...
  login: async (identifier, password) => {
    try {
      ...
      if (matchedEmp) {
        ...
        let storedPhoto = matchedEmp.UPhoto || matchedEmp.avatar || null;

        // Fetch UPhoto from users table if available
        if (db) {
          try {
            const userDocSnap = await getDoc(doc(db, 'users', uid));
            if (userDocSnap.exists() && userDocSnap.data().UPhoto) {
              storedPhoto = userDocSnap.data().UPhoto;
            }
          } catch (e) {
            console.warn("Error fetching UPhoto from users table:", e.message);
          }
        }
