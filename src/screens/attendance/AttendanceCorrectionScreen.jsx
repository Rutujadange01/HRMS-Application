...
import { Clock, PlusCircle, CheckCircle2, XCircle, ArrowLeft, ShieldCheck, Calendar, FileText, ArrowRight, AlertTriangle, Trash2, Edit2 } from 'lucide-react-native';
...
export const AttendanceCorrectionScreen = ({ navigation }) => {
  ...
  const [addedCheckIn, setAddedCheckIn] = useState('');
  const [addedCheckOut, setAddedCheckOut] = useState('');
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
      const payload = {
        ID: editingItemId || ('ac_' + Date.now()),
        CompanyID: profile?.companyId || '',
        UserID: profile?.uid || profile?.UserID || '',
        employeeId: profile?.uid || profile?.UserID || '',
      ...
      Alert.alert('Success', `Attendance Correction request ${editingItemId ? 'updated' : 'submitted'} for approval.`);
      setReason('');
  ...
  const handleEditMyRequest = (crItem) => {
    setEditingItemId(crItem.ID || crItem.id);
    setOriginalDate(crItem.Original_Date || new Date().toISOString().split('T')[0]);
    setCorrectionDate(crItem.Correction_Date || crItem.Original_Date || new Date().toISOString().split('T')[0]);
    setCorrectionType(crItem.correction_type || 'In');
    setAddedCheckIn(crItem.Added_CheckIn === '--:--' ? '' : (crItem.Added_CheckIn || ''));
    setAddedCheckOut(crItem.Added_CheckOut === '--:--' ? '' : (crItem.Added_CheckOut || ''));
    setRequestedCheckIn(crItem.Requested_CheckIn === '--:--' ? '' : (crItem.Requested_CheckIn || ''));
    setRequestedCheckOut(crItem.Requested_CheckOut === '--:--' ? '' : (crItem.Requested_CheckOut || ''));
    setReason(crItem.Reason || '');
  ...
  const handleCancelMyRequest = async (crItem) => {
    try {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.titleRow}>
        <Text style={styles.screenTitle} numberOfLines={1}>Correction Requests</Text>
      ...
      {filteredRequests.length === 0 ? (
        ...
        [...filteredRequests].sort((a,b) => new Date(b.CreatedDate || 0) - new Date(a.CreatedDate || 0)).map((item) => {
          ...
          return (
            <View key={item.ID || item.id} style={styles.requestCard}>
              ...
              <View style={styles.dateComparisonRow}>
                ...
                <View style={styles.dateCol}>
                  <Text style={styles.dateColLabel}>Correction Date</Text>
              ...
              {isRejected && (
                <View style={styles.rejectionBox}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.rejectionText, { marginBottom: 2 }]}>Rejected on: {(item.Rejecteddate || item.UpdatedDate || '').split('T')[0]}</Text>
                    {!!item.Rejection_Reason && <Text style={styles.rejectionText}>Reason: {item.Rejection_Reason}</Text>}
                  </View>
                </View>
              )}
              ...
              {/* Employee Cancel / Edit Action */}
              {isEmployee && isPending && (
                <View style={styles.userActionRow}>
                  <TouchableOpacity style={[styles.iconActionBtn, { backgroundColor: 'rgba(59, 130, 246, 0.1)' }]} onPress={() => handleEditMyRequest(item)}>
                    <Edit2 size={16} color={COLORS.primary} />
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.iconActionBtn, { backgroundColor: 'rgba(239, 68, 68, 0.1)' }]} onPress={() => handleCancelMyRequest(item)}>
