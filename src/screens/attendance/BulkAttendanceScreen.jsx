...
import { Clock, Users, CheckSquare, Square, ArrowLeft, ShieldCheck, CheckCircle2, UserCheck, Calendar, Filter, Building2, ChevronDown } from 'lucide-react-native';
...
export const BulkAttendanceScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        console.warn("Failed to load companies in BulkAttendance", err);
      }
    };
    fetchCompanies();
  }, [profile]);
  ...
  const companyId = selectedCompanyId || profile?.companyId || 'comp_01';
  ...
  const handleProcessBulkPunch = async () => {
    ...
    try {
      ...
      await Promise.all(newLogs.map(log => attendanceService.addAttendanceLog(log)));
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <Text style={{ fontSize: 14, color: COLORS.textPrimary, fontWeight: '600' }}>
                  {availableCompanies.find(c => (c.id || c.CompanyID) === selectedCompanyId)?.CompanyName || 'Select a company...'}
                </Text>
              </View>
              ...
              <View style={styles.dropdownMenu}>
                {availableCompanies.map(c => {
                  ...
                  return (
                    <TouchableOpacity 
                      key={cId}
                      style={[styles.dropdownItem, selectedCompanyId === cId && styles.dropdownItemActive]}
                      onPress={() => {
                        setSelectedCompanyId(cId);
                        setShowCompanyDropdown(false);
                      }}
                    >
                      <Text style={[styles.dropdownItemText, selectedCompanyId === cId && styles.dropdownItemTextActive]}>
                        {c.CompanyName}
                      </Text>
                    </TouchableOpacity>
                  );
      ...
      <View style={[styles.empListSection, { zIndex: 10 }]}>
