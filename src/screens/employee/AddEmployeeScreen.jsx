    ...
    const fetchCompanies = async () => {
      try {
        ...
        const myComps = comps.filter(c => {
          ...
          return creatorId === myUserId || c.id === 'comp_01'; // Fallback to comp_01 if admin
        });
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const userPayload = {
        ...
        IsFaceEnrolled: true,
        MonthlyPay: Number(monthlyPayAmt) || 0,
        CreatedDate: new Date().toISOString(),
        UpdatedDate: new Date().toISOString()
      };

      console.log("Saving Employee JSON:", JSON.stringify(userPayload, null, 2));
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        ...
        {availableCompanies.length > 0 && (
          <>
            <Text style={styles.sectionLabel}>Assign to Company</Text>
            <View style={styles.chipRow}>
              {availableCompanies.map((c) => (
                <TouchableOpacity
                  key={c.id || c.CompanyID}
                  style={[styles.chip, selectedCompanyId === (c.id || c.CompanyID) && styles.chipActive]}
                  onPress={() => setSelectedCompanyId(c.id || c.CompanyID)}
                >
                  <Text style={[styles.chipText, selectedCompanyId === (c.id || c.CompanyID) && styles.chipTextActive]}>
                    {c.CompanyName || c.name || c.Company_Name || c.id}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
