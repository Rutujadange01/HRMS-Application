...
export const CompanyProfileScreen = ({ navigation }) => {
  ...
  const [companyName, setCompanyName] = useState(company?.CompanyName || company?.name || '');
  const [address1, setAddress1] = useState(company?.Address1 || company?.address || '');
  const [phoneNo, setPhoneNo] = useState(company?.PhoneNo || company?.phone || '');
  const [email, setEmail] = useState(company?.Email || company?.email || '');
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      // Check for unique Company Code
      const allComps = await companyService.getAllCompanies();
      const codeExists = allComps.find(c => c.CompanyCode?.toLowerCase() === companyCode.toLowerCase() && (c.id !== companyId && c.CompanyID !== companyId));
      if (codeExists) {
        setLoading(false);
        Alert.alert('Duplicate Code', 'This Company Code is already present. Please enter a unique or different code.');
        return;
      }
  ...
  const handleCreateNewId = () => {
    ...
    Alert.alert("New Company Session Started", `Please enter the new company's details. (Internal ID: ${newId})`);
  };
