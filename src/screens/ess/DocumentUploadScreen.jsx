...
export const DocumentUploadScreen = ({ navigation }) => {
  ...
  try {
    console.log('Starting document upload for types:', typesToUpload);
    for (const docId of typesToUpload) {
      const file = localFiles[docId];
      const docDef = REQUIRED_DOCS.find(d => d.id === docId);
      console.log('Uploading', docDef.label, 'file:', file);

      // Upload to Firebase Storage
      const uploadedUrl = await documentService.uploadFile(file.uri, file.name, userId);
      ...
      console.log('Adding attachment with newDoc:', newDoc);
