...
export const EssDashboardScreen = ({ navigation }) => {
  ...
  const currentUPhoto = profile?.UPhoto || currentEmpRecord?.UPhoto || profile?.avatar || currentEmpRecord?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile?.name || 'User')}&background=F15E8C&color=fff`;
  ...
  const handleSaveUPhoto = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        "Your biometric face photo (UPhoto) has been saved into user record. Punch In / Punch Out will now match against this stored photo."
      );
    } catch (e) {
